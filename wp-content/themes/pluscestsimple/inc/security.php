...
add_action( 'send_headers', function () {
	...
	// systématique en sortie, JSON_HEX_TAG sur JSON-LD, HSTS+X-Frame+Permissions-Policy en place.
	...
	$headers = apply_filters( 'pcs_security_headers', [
		...
		'Content-Security-Policy' => apply_filters( 'pcs_csp', $default_csp ),
	] );
...
add_filter( 'login_errors', function () {
	return __( 'Identifiants invalides.', 'pluscestsimple' );
} );
...
add_filter( 'wp_login_errors', function ( $errors ) {
	if ( is_wp_error( $errors ) ) {
		$errors = new WP_Error();
		$errors->add( 'invalid', __( 'Identifiants invalides.', 'pluscestsimple' ) );
	}
	return $errors;
}, 99 );
...
add_action( 'init', function () {
	add_rewrite_rule( '^\.well-known/security\.txt$', 'index.php?pcs_security_txt=1', 'top' );
} );

add_filter( 'query_vars', function ( $vars ) {
	$vars[] = 'pcs_security_txt';
	return $vars;
} );

add_action( 'template_redirect', function () {
	if ( ! get_query_var( 'pcs_security_txt' ) ) { return; }
	$contact = apply_filters( 'pcs_security_contact', 'mailto:' . ( get_option( 'admin_email' ) ?: 'security@' . wp_parse_url( home_url(), PHP_URL_HOST ) ) );
...
add_filter( 'rest_pre_dispatch', function ( $result, $server, $request ) {
	if ( $request->get_route() !== '/pcs/v1/submit' ) { return $result; }
	if ( is_user_logged_in() ) { return $result; } // admins / editors exempt

	$cap = (int) apply_filters( 'pcs_global_rate_cap', 60 ); // 60 submissions / hour total
	...
	$bucket = function_exists( 'pcs_rate_hit' )
		? pcs_rate_hit( 'pcs_rl_global', HOUR_IN_SECONDS )
		: ( (int) get_transient( 'pcs_rl_global' ) + 1 );
	if ( $bucket > $cap ) {
		return new WP_REST_Response( [ 'ok' => false, 'error' => 'rate_limited_global', 'retry_after' => 3600 ], 429 );
	}
	if ( ! function_exists( 'pcs_rate_hit' ) ) {
		set_transient( 'pcs_rl_global', $bucket, HOUR_IN_SECONDS );
	}
	return $result;
}, 10, 3 );
...
function pcs_safe_email_header( string $email ): string {
...
function pcs_csv_escape( $value ): string {
	$s = (string) $value;
	if ( $s === '' ) { return ''; }
	$first = $s[0];
...
// Derrière Cloudflare/OVH proxy/load balancer, REMOTE_ADDR vaut l'IP du proxy
// ⇒ tous les visiteurs partagent le même compteur de rate-limit (DoS auto-infligé).
// On lit CF-Connecting-IP en priorité, puis X-Forwarded-For (premier IP de la chaîne),
// avec fallback REMOTE_ADDR. Validation FILTER_VALIDATE_IP pour éviter spoof de header.

function pcs_client_ip(): string {
...
function pcs_hash_ip( string $ip ): string {
	if ( $ip === '' ) { return ''; }
...
function pcs_rate_hit( string $key, int $ttl_seconds ): int {
	if ( wp_using_ext_object_cache() ) {
		$hit = wp_cache_get( $key, 'pcs_rl' );
		if ( false === $hit ) {
			wp_cache_set( $key, 1, 'pcs_rl', $ttl_seconds );
			return 1;
		}
		return (int) wp_cache_incr( $key, 1, 'pcs_rl' );
	}
