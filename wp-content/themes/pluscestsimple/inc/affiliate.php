...
add_filter( 'the_content', function ( $content ) {
	...
	if ( $has_affiliate && apply_filters( 'arw_pulse_auto_disclosure', true ) && strpos( $content, 'arw-disclosure' ) === false ) {
		$disclosure = arw_pulse_disclosure_markup();
 ...
 */
function arw_pulse_disclosure_markup() {
	$text = apply_filters(
		'arw_pulse_disclosure_text',
		__( 'Cet article contient des liens d\'affiliation. Si vous achetez via ces liens, nous pouvons percevoir une commission sans coût supplémentaire pour vous. Cela nous aide à maintenir le site.', 'arw-pulse' )
	);
	return '<aside class="arw-disclosure" role="note">' . esc_html( $text ) . '</aside>';
}

/**
 * Append UTM to a URL. Usage in templates or patterns.
 */
function arw_pulse_utm( $url, $campaign = '', $medium = 'affiliate' ) {
	$params = [
