<?php
/**
 * Image helpers: register sizes, generate <picture> with AVIF/WebP fallback when available.
 */
if ( ! defined( 'ABSPATH' ) ) { exit; }

add_action( 'after_setup_theme', function () {
	set_post_thumbnail_size( 1200, 630, true ); // OG default.
	add_image_size( 'arw-hero', 1600, 900, true );
	add_image_size( 'arw-card', 800, 450, true );
	add_image_size( 'arw-square', 600, 600, true );
	add_image_size( 'arw-vertical', 600, 900, true );
} );
 ...
 */
function arw_pulse_picture( $attachment_id, $size = 'large', $attrs = [] ) {
...
add_filter( 'post_thumbnail_html', function ( $html, $post_id, $thumbnail_id, $size, $attr ) {
	...
	return arw_pulse_picture( $thumbnail_id, $size, $attrs );
}, 10, 5 );
