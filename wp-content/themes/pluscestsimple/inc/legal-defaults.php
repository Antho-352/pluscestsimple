...
add_action( 'admin_menu', function () {
	add_submenu_page(
		...
		__( 'Mentions légales', 'arw-pulse' ),
		__( 'Mentions légales', 'arw-pulse' ),
		'manage_options',
		'arw-legal',
		'arw_pulse_render_legal_settings'
	);
} );

add_action( 'admin_init', function () {
	register_setting( 'arw_legal', 'arw_legal', [
		'type'              => 'array',
		'sanitize_callback' => 'arw_pulse_sanitize_legal',
		'default'           => arw_pulse_legal_defaults(),
	] );
} );
 ...
 */
function arw_pulse_legal_defaults() {
...
function arw_pulse_sanitize_legal( $input ) {
	$clean = [];
	foreach ( arw_pulse_legal_defaults() as $k => $_default ) {
...
function arw_pulse_render_legal_settings() {
	$v = wp_parse_args( get_option( 'arw_legal', [] ), arw_pulse_legal_defaults() );
	?>
	<div class="wrap">
		<h1><?php esc_html_e( 'Mentions légales — valeurs du site', 'arw-pulse' ); ?></h1>
		<p><?php esc_html_e( 'Ces champs alimentent les patterns légaux (mentions, confidentialité, cookies).', 'arw-pulse' ); ?></p>
		<form method="post" action="options.php">
			<?php settings_fields( 'arw_legal' ); ?>
			<table class="form-table" role="presentation">
				<?php foreach ( $v as $k => $val ) : ?>
					<tr>
						<th><label><?php echo esc_html( ucwords( str_replace( '_', ' ', $k ) ) ); ?></label></th>
						<td><input type="text" class="regular-text" name="arw_legal[<?php echo esc_attr( $k ); ?>]" value="<?php echo esc_attr( $val ); ?>"></td>
					</tr>
				<?php endforeach; ?>
			</table>
			<?php submit_button(); ?>
 ...
 * Accessor.
 */
function arw_pulse_legal( $key = null ) {
	$all = wp_parse_args( get_option( 'arw_legal', [] ), arw_pulse_legal_defaults() );
 ...
 */
function arw_pulse_fill_legal( $text ) {
	$vals = arw_pulse_legal();
	foreach ( $vals as $k => $v ) {
