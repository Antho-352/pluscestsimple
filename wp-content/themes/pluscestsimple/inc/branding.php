 ...
 *
 * - Logo: inline SVG with site name + signature mark.
 ...
 * Uses currentColor so it adapts to dark/light header.
 */
function pcs_default_logo_svg() {
...
add_filter( 'render_block_core/site-logo', function ( $content, $block ) {
	...
	return sprintf(
		'<div class="wp-block-site-logo pcs-default-logo"><a href="%s" class="custom-logo-link" rel="home" aria-label="%s">%s</a></div>',
		esc_url( home_url( '/' ) ),
		esc_attr( get_bloginfo( 'name' ) ),
		pcs_default_logo_svg()
	);
}, 10, 2 );
...
add_action( 'wp_head', function () {
	...
	$favicon = PCS_DIR . '/assets/images/favicon.svg';
	if ( file_exists( $favicon ) ) {
		printf(
			'<link rel="icon" href="%s" type="image/svg+xml">' . "\n",
			esc_url( PCS_URI . '/assets/images/favicon.svg' )
		);
	}
	// PNG fallback for older browsers + apple-touch-icon.
	$png = PCS_DIR . '/assets/images/favicon-512.png';
	if ( file_exists( $png ) ) {
		printf(
			'<link rel="icon" type="image/png" sizes="512x512" href="%s">' . "\n",
			esc_url( PCS_URI . '/assets/images/favicon-512.png' )
		);
		printf(
...
add_action( 'init', function () {
	add_rewrite_rule( '^manifest\.webmanifest$', 'index.php?pcs_manifest=1', 'top' );
} );
...
add_action( 'template_redirect', function () {
	if ( ! get_query_var( 'pcs_manifest' ) ) { return; }
	$icon_id  = (int) get_option( 'site_icon' );
	$icon_url = '';
	if ( $icon_id ) {
	...
	if ( ! $icon_url && file_exists( PCS_DIR . '/assets/images/favicon-512.png' ) ) {
		$icon_url = PCS_URI . '/assets/images/favicon-512.png';
	}
	$manifest = [
