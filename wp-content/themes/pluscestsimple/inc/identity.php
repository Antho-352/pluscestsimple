<?php
/**
 * Identité & Social — UI admin pour les valeurs site-specific.
 *
 * Couvre :
...
const PCS_IDENT_OPT = 'pcs_identity';

// ─── Defaults ────────────────────────────────────────────────────────────────

function pcs_identity_defaults(): array {
	return [
		'twitter_site'       => '',
		'twitter_creator'    => '',
		'social_twitter'     => '',
...
function pcs_identity(): array {
	return wp_parse_args( (array) get_option( PCS_IDENT_OPT, [] ), pcs_identity_defaults() );
}
...
add_action( 'admin_menu', function () {
	add_submenu_page(
		'options-general.php',
		__( 'Identité & Social', 'pluscestsimple' ),
		__( 'Identité & Social', 'pluscestsimple' ),
		'manage_options',
		'pcs-identity',
		'pcs_render_identity_page'
	);
} );
...
add_action( 'admin_init', function () {
	register_setting( 'pcs_identity_grp', PCS_IDENT_OPT, [
		'type'              => 'array',
		'sanitize_callback' => 'pcs_sanitize_identity',
		'default'           => pcs_identity_defaults(),
	] );
} );

function pcs_sanitize_identity( $input ): array {
	$in    = is_array( $input ) ? $input : [];
	$clean = pcs_identity_defaults();
...
add_filter( 'pcs_twitter_site', function ( $default ) {
	$v = pcs_identity()['twitter_site'];
	return $v ? '@' . $v : $default;
}, 5 );

add_filter( 'pcs_twitter_creator', function ( $default ) {
	$v = pcs_identity()['twitter_creator'];
	return $v ? '@' . $v : $default;
}, 5 );

add_filter( 'pcs_organization_same_as', function ( $defaults ) {
	$v = pcs_identity();
	$urls = array_filter( [
		$v['social_twitter'],
		$v['social_instagram'],
...
add_filter( 'pcs_default_og_image', function ( $default ) {
	$id = (int) pcs_identity()['og_image_id'];
	if ( ! $id ) { return $default; }
	$src = wp_get_attachment_image_src( $id, 'large' );
	return $src ? $src[0] : $default;
}, 5 );

add_filter( 'pcs_disclosure_text', function ( $default ) {
	$v = pcs_identity()['disclosure_text'];
	return $v ?: $default;
}, 5 );

add_filter( 'pcs_form_to', function ( $email, $form_type ) {
	$v   = pcs_identity();
	$key = 'form_to_' . str_replace( '-inquiry', '', str_replace( '-', '_', $form_type ) );
	return ! empty( $v[ $key ] ) ? $v[ $key ] : $email;
}, 5, 2 );

add_filter( 'pcs_dns_prefetch', function ( $defaults ) {
	$raw = (string) pcs_identity()['dns_prefetch'];
...
function pcs_render_identity_page(): void {
	$v = pcs_identity();
	...
	?>
	<div class="wrap">
		<h1><?php esc_html_e( 'Identité & Social', 'pluscestsimple' ); ?></h1>
		<p style="max-width:780px;color:#50575e">
			<?php esc_html_e( 'Ces champs alimentent automatiquement le schema Organization, les meta OG/Twitter, le routing email des formulaires et le disclaimer d\'affiliation. Un plugin custom peut toujours les écraser via filtre PHP.', 'pluscestsimple' ); ?>
		</p>

		<form method="post" action="options.php">
			<?php settings_fields( 'pcs_identity_grp' ); ?>

			<h2><?php esc_html_e( 'Twitter / X', 'pluscestsimple' ); ?></h2>
			<table class="form-table" role="presentation">
				<tr>
					<th><label>Handle du site</label></th>
					<td>
						<span style="color:#787c82">@</span>
						<input type="text" name="<?php echo esc_attr( $o ); ?>[twitter_site]" value="<?php echo esc_attr( $v['twitter_site'] ); ?>" placeholder="monsite" maxlength="15">
			...
			<h2><?php esc_html_e( 'Réseaux sociaux', 'pluscestsimple' ); ?></h2>
			<p class="description" style="margin-bottom:12px"><?php esc_html_e( 'URLs complètes. Injectées dans le schema Organization (sameAs) — renforce la confiance Google.', 'pluscestsimple' ); ?></p>
			<table class="form-table" role="presentation">
				<?php foreach ( [
			...
			<h2><?php esc_html_e( 'Image sociale par défaut (OG fallback)', 'pluscestsimple' ); ?></h2>
			<table class="form-table" role="presentation">
				<tr>
					<th><label>Image</label></th>
					<td>
						<div id="pcs_ident_og_prev" style="margin-bottom:8px">
							<?php if ( $og_url ) : ?>
								<img src="<?php echo esc_url( $og_url ); ?>" style="max-width:300px;height:auto;border-radius:6px;border:1px solid #dcdcde">
							<?php endif; ?>
						</div>
						<button type="button" class="button" id="pcs_ident_og_pick">Choisir une image</button>
						<button type="button" class="button" id="pcs_ident_og_clear" <?php echo $og_id ? '' : 'style="display:none"'; ?>>Retirer</button>
						<input type="hidden" id="pcs_ident_og_id" name="<?php echo esc_attr( $o ); ?>[og_image_id]" value="<?php echo (int) $og_id; ?>">
						<p class="description">Utilisée pour les partages réseaux quand un article n'a pas d'image à la une. Idéal : 1200×630 px.</p>
					</td>
				</tr>
			</table>

			<h2><?php esc_html_e( 'Disclaimer affiliation', 'pluscestsimple' ); ?></h2>
			<table class="form-table" role="presentation">
				<tr>
			...
			<h2><?php esc_html_e( 'Routing email des formulaires', 'pluscestsimple' ); ?></h2>
			<p class="description" style="margin-bottom:12px"><?php esc_html_e( 'Email destinataire selon le type de formulaire. Laisser vide → email admin du site.', 'pluscestsimple' ); ?></p>
			<table class="form-table" role="presentation">
				<?php foreach ( [
			...
			<h2><?php esc_html_e( 'DNS prefetch', 'pluscestsimple' ); ?></h2>
			<table class="form-table" role="presentation">
				<tr>
	...
	<script>
	(function(){
		var pick=document.getElementById('pcs_ident_og_pick'),
			clear=document.getElementById('pcs_ident_og_clear'),
			prev=document.getElementById('pcs_ident_og_prev'),
			idF=document.getElementById('pcs_ident_og_id'),
			frame;
