...
add_action( 'wp_head', function () {
	...
	// Use pcs-card (800×600) — matches card display size.
	$src = wp_get_attachment_image_src( $thumb_id, 'pcs-card' )
	...
	printf(
		...
		esc_url( $src[0] ),
		esc_attr( wp_get_attachment_image_srcset( $thumb_id, 'pcs-card' ) ?: '' )
	);
}, 3 );
...
add_action( 'wp_head', function () {
	...
	foreach ( [ 'inter-var', 'space-grotesk-var' ] as $name ) {
		$path = PCS_DIR . '/assets/fonts/' . $name . '.woff2';
		if ( file_exists( $path ) ) {
			$fonts[] = PCS_URI . '/assets/fonts/' . $name . '.woff2';
		}
	}
	$fonts = apply_filters( 'pcs_preload_fonts', $fonts );
...
add_action( 'wp_head', function () {
	$path = PCS_DIR . '/assets/css/critical.css';
	...
	if ( $css ) {
		...
		echo '<style id="pluscestsimple-critical">' . $css . '</style>' . "\n";
	}
}, 5 );
...
add_filter( 'script_loader_tag', function ( $tag, $handle ) {
	$defer = apply_filters( 'pcs_defer_scripts', [
		'wp-embed',
	] );
...
add_filter( 'wp_resource_hints', function ( $hints, $relation_type ) {
	if ( 'dns-prefetch' === $relation_type ) {
		$hints = array_merge( $hints, apply_filters( 'pcs_dns_prefetch', [] ) );
	}
	return $hints;
}, 10, 2 );
...
const PCS_HTACCESS_VERSION = '1';

add_action( 'admin_init', function () {
	$stored = get_option( 'pcs_htaccess_cache_v' );
	if ( $stored === PCS_HTACCESS_VERSION ) { return; }
	if ( ! function_exists( 'insert_with_markers' ) ) {
		require_once ABSPATH . 'wp-admin/includes/misc.php';
	}
	if ( pcs_write_cache_htaccess() ) {
		update_option( 'pcs_htaccess_cache_v', PCS_HTACCESS_VERSION );
	}
} );

function pcs_write_cache_htaccess(): bool {
	...
	return (bool) insert_with_markers( $htaccess, 'Plus c\'est simple Cache', $rules );
}
...
const PCS_HTACCESS_HARDEN_VERSION = '1';

add_action( 'admin_init', function () {
	$stored = get_option( 'pcs_htaccess_harden_v' );
	if ( $stored === PCS_HTACCESS_HARDEN_VERSION ) { return; }
	if ( ! function_exists( 'insert_with_markers' ) ) {
		require_once ABSPATH . 'wp-admin/includes/misc.php';
	}
	if ( pcs_write_harden_htaccess() ) {
		update_option( 'pcs_htaccess_harden_v', PCS_HTACCESS_HARDEN_VERSION );
	}
} );

function pcs_write_harden_htaccess(): bool {
	if ( ! function_exists( 'get_home_path' ) ) {
		require_once ABSPATH . 'wp-admin/includes/file.php';
	}
	$home_path = function_exists( 'get_home_path' ) ? get_home_path() : ABSPATH;
	...
	return (bool) insert_with_markers( $htaccess, 'Plus c\'est simple Hardening', $rules );
}
