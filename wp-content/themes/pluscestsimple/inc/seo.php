 ...
 */
function arw_pulse_normalize_social_text( string $s ): string {
...
function arw_pulse_meta_description() {
	$desc = '';
	if ( is_singular() ) {
		$post = get_queried_object();
		$desc = get_post_meta( $post->ID, '_arw_meta_description', true );
		...
		if ( ! $desc ) {
			// Si le contenu n'est qu'un shortcode (ex: [arw_compatibilimetre]),
			// l'expansion via do_shortcode() évite que le snippet exposé soit le shortcode brut.
		...
		$desc = term_description( $term )
			...
			: sprintf( __( 'Articles dans la catégorie %s.', 'arw-pulse' ), $term->name );
	} elseif ( is_author() ) {
		...
		if ( ! $desc ) {
			...
			$desc = sprintf( __( 'Découvrez %s : actualités, guides et ressources éditoriales.', 'arw-pulse' ), get_bloginfo( 'name' ) );
		}
	} elseif ( is_search() ) {
		$desc = sprintf( __( 'Résultats de recherche pour « %s »', 'arw-pulse' ), get_search_query() );
	}
	$desc = wp_strip_all_tags( $desc );
	// Hard cap at 160 to avoid Google snippet truncation.
	if ( mb_strlen( $desc ) > 160 ) { $desc = rtrim( mb_substr( $desc, 0, 157 ) ) . '…'; }
	return apply_filters( 'arw_pulse_meta_description', $desc );
}
 ...
 */
function arw_pulse_canonical_url() {
 ...
 */
function arw_pulse_og_image() {
	if ( is_singular() ) {
		$post = get_queried_object();
		$override = (int) get_post_meta( $post->ID, '_arw_og_image_id', true );
		if ( $override ) {
	...
	$default = apply_filters( 'arw_pulse_default_og_image', '' );
 ...
 */
function arw_pulse_og_image_dimensions( string $url ): array {
 ...
 */
function arw_pulse_twitter_site()    { return apply_filters( 'arw_pulse_twitter_site',    '' ); }
function arw_pulse_twitter_creator() { return apply_filters( 'arw_pulse_twitter_creator', '' ); }
...
add_action( 'wp_head', function () {
	$desc  = arw_pulse_meta_description();
	$url   = arw_pulse_canonical_url();
	...
	$noindex = false;
	if ( is_singular() ) {
		$noindex = (bool) get_post_meta( get_queried_object_id(), '_arw_noindex', true );
	}
	if ( is_search() || is_404() ) { $noindex = true; }
	// Author archives noindex global — pas de valeur SEO sur sites éditoriaux mono-auteur.
	if ( is_author() && apply_filters( 'arw_pulse_author_archives_noindex', true ) ) {
		$noindex = true;
	}
	$default_index = apply_filters( 'arw_pulse_robots_meta_index', 'index, follow, max-image-preview:large, max-snippet:-1, max-video-preview:-1' );
	...
	$og_title       = apply_filters( 'arw_pulse_og_title', $title );
	$og_description = apply_filters( 'arw_pulse_og_description', $desc );
	printf( '<meta property="og:title" content="%s">'     . "\n", esc_attr( $og_title ) );
	printf( '<meta property="og:description" content="%s">' . "\n", esc_attr( $og_description ) );
	printf( '<meta property="og:url" content="%s">'       . "\n", esc_url( $url ) );
	$og_type = 'website';
	if ( is_singular( 'arw_spot' ) || is_singular( 'arw_trail' ) ) {
		$og_type = 'place';
	} elseif ( is_singular() && ! is_page() ) {
		// Posts standards + tous les CPT (article, recette, règle compat, etc.).
		$og_type = 'article';
	}
	$og_type = apply_filters( 'arw_pulse_og_type', $og_type );
	printf( '<meta property="og:type" content="%s">' . "\n", esc_attr( $og_type ) );

	$og_image = arw_pulse_og_image();
	if ( $og_image ) {
		[ $og_w, $og_h ] = arw_pulse_og_image_dimensions( $og_image );
	...
	// Audit 2026-05-14 (M4) : normalisation unicode pour éviter homograph/RTL-override dans preview.
	printf( '<meta name="twitter:card" content="%s">'  . "\n", 'summary_large_image' );
	printf( '<meta name="twitter:title" content="%s">' . "\n", esc_attr( arw_pulse_normalize_social_text( $title ) ) );
	printf( '<meta name="twitter:description" content="%s">' . "\n", esc_attr( arw_pulse_normalize_social_text( $desc ) ) );
	if ( $og_image ) { printf( '<meta name="twitter:image" content="%s">'       . "\n", esc_url( $og_image ) ); }

	// Sanitiser handles Twitter : forcer "@" en préfixe (sinon meta invalide).
	$tw_site    = trim( (string) arw_pulse_twitter_site(), '@ ' );
	$tw_creator = trim( (string) arw_pulse_twitter_creator(), '@ ' );
...
add_action( 'template_redirect', function () {
	...
	$canonical = arw_pulse_canonical_url();
	if ( ! $canonical ) { return; }
	...
	if ( $req_path && $can_path && rtrim( $req_path, '/' ) === rtrim( $can_path, '/' ) && $req_path !== $can_path ) {
		...
		if ( $req_query ) {
			...
			$allowed = apply_filters( 'arw_pulse_redirect_allowed_args', [
...
add_action( 'init', function () {
	$fields = [
		'_arw_meta_title'       => 'string',
		'_arw_meta_description' => 'string',
		'_arw_og_image_id'      => 'integer',
		'_arw_noindex'          => 'boolean',
		'_arw_focus_keyword'    => 'string',
	];
...
add_action( 'add_meta_boxes', function () {
	...
	add_meta_box( 'arw_seo', 'SEO', 'arw_pulse_seo_meta_box', $types, 'normal', 'high' );
} );

function arw_pulse_seo_meta_box( WP_Post $post ): void {
	wp_nonce_field( 'arw_seo_save', 'arw_seo_nonce' );
	$title    = (string) get_post_meta( $post->ID, '_arw_meta_title', true );
	$desc     = (string) get_post_meta( $post->ID, '_arw_meta_description', true );
	$og_id    = (int)    get_post_meta( $post->ID, '_arw_og_image_id', true );
	$noindex  = (bool)   get_post_meta( $post->ID, '_arw_noindex', true );
	$keyword  = (string) get_post_meta( $post->ID, '_arw_focus_keyword', true );
	...
	?>
	<style>
		#arw_seo{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;font-size:13px}
		#arw_seo .f{margin-bottom:14px}
		#arw_seo label{display:block;font-weight:600;margin-bottom:4px}
		#arw_seo label span{font-weight:400;color:#787c82}
		#arw_seo input[type=text],#arw_seo textarea{width:100%;box-sizing:border-box;border:1px solid #8c8f94;border-radius:3px;padding:6px 8px;font-size:13px;line-height:1.5}
		#arw_seo textarea{resize:vertical;min-height:68px}
		#arw_seo .cnt{font-size:11px;color:#8c8f94;float:right;margin-top:3px}
		#arw_seo .cnt.warn{color:#d63638;font-weight:600}
		#arw_seo .prev{background:#f6f7f7;border:1px solid #dcdcde;border-radius:4px;padding:12px 14px;margin-top:6px;overflow:hidden}
		#arw_seo .prev-url{font-size:12px;color:#0d652d;margin-bottom:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
		#arw_seo .prev-title{font-size:18px;color:#1a0dab;line-height:1.3;margin:0 0 3px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
		#arw_seo .prev-desc{font-size:13px;color:#4d5156;line-height:1.55;margin:0;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}
		#arw_seo .og-preview{width:160px;height:84px;object-fit:cover;border-radius:4px;border:1px solid #dcdcde;margin-right:10px}
		#arw_seo .row{display:flex;align-items:center;gap:10px;flex-wrap:wrap}
		#arw_seo .btn{background:#f0f0f1;border:1px solid #8c8f94;border-radius:3px;padding:5px 10px;font-size:12px;cursor:pointer}
		#arw_seo .adv{border-top:1px solid #dcdcde;margin-top:16px;padding-top:12px}
		#arw_seo .adv summary{font-weight:600;cursor:pointer;margin-bottom:10px}
	</style>

	<div class="f">
		<label for="arw_meta_title">Titre SEO <span>(défaut : «&nbsp;<?php echo esc_html( $def_title ); ?>&nbsp;»)</span></label>
		<input type="text" id="arw_meta_title" name="arw_meta_title"
			value="<?php echo esc_attr( $title ); ?>"
			placeholder="<?php echo esc_attr( $def_title ); ?>" maxlength="120">
		<span class="cnt" id="arw_tc"><?php echo mb_strlen( $title ); ?>/60</span>
	</div>

	<div class="f">
		<label for="arw_meta_description">Meta description <span>(160 chars max)</span></label>
		<textarea id="arw_meta_description" name="arw_meta_description"
			maxlength="160" rows="3"
			placeholder="<?php echo esc_attr( mb_substr( $def_desc, 0, 160 ) ); ?>"><?php echo esc_textarea( $desc ); ?></textarea>
		<span class="cnt" id="arw_dc"><?php echo mb_strlen( $desc ); ?>/160</span>
	</div>

	<div class="prev">
		<div class="prev-url"><?php echo esc_html( $permalink ); ?></div>
		<div class="prev-title" id="arw_pt"><?php echo esc_html( $title ?: $def_title ); ?></div>
		<p  class="prev-desc"  id="arw_pd"><?php echo esc_html( $desc  ?: mb_substr( $def_desc, 0, 160 ) ); ?></p>
	</div>
	...
	<details class="adv">
		...
		<div class="f">
			...
			<div class="row">
				<img id="arw_og_prev" class="og-preview" src="<?php echo esc_url( $og_url ); ?>" alt="" style="<?php echo $og_url ? '' : 'display:none'; ?>">
				<button type="button" class="btn" id="arw_og_pick">Choisir</button>
				<button type="button" class="btn" id="arw_og_clear" style="<?php echo $og_id ? '' : 'display:none'; ?>">Retirer</button>
				<input type="hidden" id="arw_og_id" name="arw_og_image_id" value="<?php echo (int) $og_id; ?>">
			</div>
		</div>

		<div class="f">
			<label><input type="checkbox" name="arw_noindex" value="1" <?php checked( $noindex ); ?>> Noindex — retirer cette page des moteurs de recherche</label>
		</div>

		<div class="f">
			<label for="arw_focus_keyword">Mot-clé principal <span>(optionnel, pour ton suivi)</span></label>
			<input type="text" id="arw_focus_keyword" name="arw_focus_keyword" value="<?php echo esc_attr( $keyword ); ?>" maxlength="80">
		</div>
	</details>

	<script>
	(function(){
		var ti=document.getElementById('arw_meta_title'),
		    di=document.getElementById('arw_meta_description'),
		    tc=document.getElementById('arw_tc'),
		    dc=document.getElementById('arw_dc'),
		    pt=document.getElementById('arw_pt'),
		    pd=document.getElementById('arw_pd'),
		...
		var pickBtn=document.getElementById('arw_og_pick'),
		    clearBtn=document.getElementById('arw_og_clear'),
		    prev=document.getElementById('arw_og_prev'),
		    hid=document.getElementById('arw_og_id'),
		    frame;
...
add_action( 'save_post', function ( int $post_id ): void {
	if ( empty( $_POST['arw_seo_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['arw_seo_nonce'] ) ), 'arw_seo_save' ) ) { return; }
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) { return; }
	if ( ! current_user_can( 'edit_post', $post_id ) ) { return; }

	// Text fields.
	foreach ( [
		'arw_meta_title'       => [ '_arw_meta_title',       120 ],
		'arw_meta_description' => [ '_arw_meta_description', 160 ],
		'arw_focus_keyword'    => [ '_arw_focus_keyword',     80 ],
	...
	// Image ID.
	$og_id = isset( $_POST['arw_og_image_id'] ) ? (int) $_POST['arw_og_image_id'] : 0;
	$og_id ? update_post_meta( $post_id, '_arw_og_image_id', $og_id ) : delete_post_meta( $post_id, '_arw_og_image_id' );

	// Checkbox.
	$noindex = ! empty( $_POST['arw_noindex'] );
	$noindex ? update_post_meta( $post_id, '_arw_noindex', 1 ) : delete_post_meta( $post_id, '_arw_noindex' );
} );
